...
cv::Mat pre_process_svm(const cv::Mat &src)
{
    ...
    Mat src_crop = src_tmp(boundRect[max_rect_ind]); //crop vung co bien bao
...
void* get_blah_blah(void* arg)
{
    ...
    {
        // cout << "OK" << endl;
        sign_crop = pre_process_svm(src_cp);

        // if ( (sign_crop.rows > 0) && (sign_crop.cols > 0) ) //tim duoc nguong hsv voi contour lon nhat
        // {
            // imshow("sign crop", sign_crop);
