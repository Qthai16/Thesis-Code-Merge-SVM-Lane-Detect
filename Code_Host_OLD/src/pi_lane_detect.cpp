...
void DetectLane::update(const Mat &src)
{
    ...
    Mat birdView, lane;
    Mat birdview_img = Mat::zeros(Size(240,320), CV_8UC1);
    //input: rgb ,output: binary image with massroad point
    birdview_img = preProcess(src, mass_road_BV); //sau khi qua preProcess thi massroad nay la massroad trong birdview
    // cout << "size bev" << birdview_img.size() << endl;
    mass_road_src = MassOfRoad(src);
    //input: binary , ouput: vector<Mat> 
    vector<Mat> layers1 = splitLayer(birdview_img);
    vector< vector<Point> > points1 = centerRoadSide(layers1);

    birdView = Mat::zeros(birdview_img.size(), CV_8UC3);
    lane = Mat::zeros(birdview_img.size(), CV_8UC3);
