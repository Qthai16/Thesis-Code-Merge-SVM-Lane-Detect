...
{
    static cv::Point null = cv::Point(-1,-1);
    bool cant_see_lane = false;
    float cur_angle, udk;
    uint16_t duty = 75;
    ...
    if (flag == 2) // binh thuong
    {
        ...
        else if ( (center_right == null) && (center_left == null) ) //mat ca 2 lane
        {
            cant_see_lane = true;
        }
    }
    ...
    if (cant_see_lane == false){
        duty = Convert_to_DutyCycles(udk, cur_angle);
        *(p_data + 1) = 40;
    }
    else if (cant_see_lane ==true){
        duty = 75;
        *(p_data + 1) = 50;
    }
#ifdef PRINT_DEBUG
